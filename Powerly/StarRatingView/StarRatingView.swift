...
protocol RatingDelegate: AnyObject {
    func changeRating(rating: Float)
}
...
class StarRatingView: UIView {
    ...
    fileprivate func setupView(rating: Float, color: UIColor, starRounding: StarRounding) {
        ...
        guard let viewFromNib = nib.instantiate(withOwner: self, options: nil)[0] as? StarRatingStackView else {
            return
        }
    ...
    fileprivate func setStarsFor(rating: Float) {
        ...
        for count in 1...5 {
            let iFloat = Float(count)
            switch starRounding {
            case .roundToHalfStar:
                starImageViews[count-1]!.image = rating >= iFloat-0.25 ? fullStarImage :
                                            (rating >= iFloat-0.75 ? halfStarImage : emptyStarImage)
                
            case .ceilToHalfStar:
                starImageViews[count-1]!.image = rating > iFloat-0.5 ? fullStarImage :
                                            (rating > iFloat-1 ? halfStarImage : emptyStarImage)
                
            case .floorToHalfStar:
                starImageViews[count-1]!.image = rating >= iFloat ? fullStarImage :
                                            (rating >= iFloat-0.5 ? halfStarImage : emptyStarImage)
                
            case .roundToFullStar:
                starImageViews[count-1]!.image = rating >= iFloat-0.5 ? fullStarImage : emptyStarImage
                
            case .ceilToFullStar:
                starImageViews[count-1]!.image = rating > iFloat-1 ? fullStarImage : emptyStarImage
                
            case .floorToFullStar:
                starImageViews[count-1]!.image = rating >= iFloat ? fullStarImage : emptyStarImage
            }
        }
    }
    ...
    fileprivate func touched(touch: UITouch, moveTouch: Bool) {
        ...
        guard let hss = self.hstack else { return }
        let touchX = touch.location(in: hss).x
        let ratingFromTouch = 5*touchX/hss.frame.width
        ...
        lastTouch = Date()
        self.delegate?.changeRating(rating: self.rating)
    }
}
